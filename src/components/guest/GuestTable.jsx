...
const GuestTable = ({ refreshTrigger, onEditClick, searchTerm, filterTerm, subCompanyFilter }) => { 
    ...
    const fetchData = async() => {
        try {
            ...
            const response = await api.get(`/guest?page=${currentPage}&pageSize=${itemsPerPage}&search=${searchTerm}&filter=${filterTerm}&subcompany=${subCompanyFilter || ''}`);
    ...
    const handleDelete = async (id, name) => {
        Confirm.fire({
            title: 'Hapus Data?',
            text: `Apakah Anda yakin ingin menghapus guest untuk ${name}?`,
            icon: 'warning',
                    ...
                    <tr key={`row-${index+1}`} >
                        ...
                        <td>
                            ...
                            <button className="btn-app btn-danger-app btn-sm-app"
                                onClick={() => handleDelete(emp.guest_id, emp.employee_name)}
                            >
